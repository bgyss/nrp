...
function buildShader(manifest) {
  ...
  const TI = 8; // input vec4s per cooperative weight-tile load
  const din4Max = Math.max(...dims.slice(0, -1).map(pad4));
  const wtileVec4s = TI * 4 * Math.min(OTILE, Math.max(...dims.slice(1).map(pad4)));
  ...
  for (let l = 0; l < dims.length - 1; l++) {
    ...
    if (din4 % TI !== 0) throw new Error(`layer ${l}: din4 ${din4} not divisible by TI ${TI}`);
...
`;
  ...
  return `
struct Params { n_pixels: u32, stride: u32, side: u32, pad: u32, light: vec4<f32>, };
...
${mlpCode}
  if (gid.x < params.n_pixels) {
    outputs[pixel * 3u] = softplus(${outArr}[0][0]);
    outputs[pixel * 3u + 1u] = softplus(${outArr}[0][1]);
    outputs[pixel * 3u + 2u] = softplus(${outArr}[0][2]);
  }
}
`;
}

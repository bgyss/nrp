...
**All per-seed dB numbers above and in the K1/equivalence-gate tables in
`docs/performance.md` (pre-2026-08-29) were computed with a 12-light held-out
estimator.** On Kitchen the per-light delta spread is large enough that this
estimator's standard error (~0.94–1.14 dB) exceeds the −0.5 dB gate threshold —
comparable in size to the effect the gate exists to detect. `world_sparse`'s
...
`docs/performance.md#re-read-at-96-held-out-lights-2026-08-29` for the full
comparison table (`out/r1-parity-kitchen-eq/rescore-96.json`, with

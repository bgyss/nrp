...
| S5 | Local training throughput: torch.compile / AMP / batch sweep | **done** (honest triple negative: batch↑ loses quality faster than it gains speed; bf16/fp16 autocast is a wash on MPS, catastrophic on CPU; torch.compile crashes on MPS, ~5% net loss on CPU; committed fp32/batch-4096 default stands) | `out/s5-throughput/*.json`, `docs/performance.md#local-training-throughput-torchcompile--amp--batch-sweep-scale-track-rung-s5` |
| S6 | WebGPU runtime at 1024² + device/precision config plumbing | **done** (T4/H4 harness bug fixed for large payloads; T4 1024² parity 1.5e-6, 11.3 fps p95 — misses 30fps but beats MPS's 8.6 Hz; H4 rig at 1024² parity 1.24e-5, rgb-slider 2.2ms p95, param-edit 219ms p95; device/precision plumbing complete + tested) | `out/s6-webgpu-1024/*.json`, `docs/performance.md#webgpu-runtime-at-1024-device-precision-plumbing-scale-track-rung-s6` |
